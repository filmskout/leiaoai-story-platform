-- ============================================
-- 迁移文件 4-A: Stories and Tags System
-- 假设主键是: story_id (不是 id)
-- ============================================
...
CREATE POLICY "Authenticated users can assign tags to their stories"
  ...
  USING (
    story_id IN (
      SELECT story_id FROM stories WHERE author = auth.uid()
    )
  );
