-- Migration: create_story_interactions_tables
-- Created at: 1759909755
...
-- Add unique constraints to prevent duplicate interactions
CREATE UNIQUE INDEX IF NOT EXISTS unique_user_story_like ON story_likes(story_id, user_id) WHERE user_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS unique_session_story_like ON story_likes(story_id, session_id) WHERE session_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS unique_user_story_save ON story_saves(story_id, user_id) WHERE user_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS unique_session_story_save ON story_saves(story_id, session_id) WHERE session_id IS NOT NULL;;