-- Fix RLS policies for tools table (Handles existing policies)
-- This version drops existing policies first to avoid conflicts

-- Drop ALL existing policies for tools table
...
DROP POLICY IF EXISTS "tools_insert_service_role" ON public.tools;
DROP POLICY IF EXISTS "tools_update_service_role" ON public.tools;
DROP POLICY IF EXISTS "tools_delete_service_role" ON public.tools;
