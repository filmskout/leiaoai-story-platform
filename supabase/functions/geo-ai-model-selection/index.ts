...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isChinese = /zh/i.test(acceptLanguage);

    const chatModels = [
      { id: 'deepseek', name: 'DeepSeek', enabled: true, recommended: isChinese },
      { id: 'openai', name: 'OpenAI GPT-4o', enabled: true, recommended: !isChinese },
    ...
    const recommendedModels = {
      chat: isChinese ? 'deepseek' : 'openai',
      image: 'dall-e',
    };

    const body = {
      data: {
        region: isChinese ? 'cn' : 'overseas',
        recommendedModels,
        modelConfigs: {
          chat: { available: chatModels },
          image: { available: imageModels },
        },
      }
    };
