import React, { useState, useEffect, lazy, Suspense, useRef, memo, useCallback } from 'react';
...
export interface UnifiedLoaderProps {
  ...
  // 显示模式
  mode?: 'inline' | 'fullscreen' | 'page' | 'cover';
...
// 使用记忆化组件减少不必要的重渲染
export const UnifiedLoader = memo(function UnifiedLoaderComponent({ 
  show = true,
  variant,
  mode = 'inline',
  size = 'md',
  text,
  subText = (variant || mode) === 'page' || (variant || mode) === 'fullscreen' ? '蕾奥AI正在为您思考...' : undefined,
  className,
  onComplete,
  minDuration = 1500,
  showProgress = (variant || mode) === 'cover',
  progress: customProgress,
  background = (variant || mode) === 'fullscreen' ? 'blur' : (variant || mode) === 'page' ? 'solid' : 'none',
  ariaLabel = '加载中',
  ariaLive = 'polite',
}: UnifiedLoaderProps) {
  // 兼容mode和variant属性
  const displayMode = variant || mode;
  ...
  }, [show, minDuration, onComplete, displayMode, isMounted]);
  
  // 图片切换效果 - 使用useCallback减少函数重建
  ...
  useEffect(() => {
    ...
    intervalRef.current = window.setInterval(updateImageIndex, 200); // 每200ms切换一次图片
    
    return () => {
      if (intervalRef.current) window.clearInterval(intervalRef.current);
    };
  }, [show, updateImageIndex]);
  ...
  }, [show, showProgress, customProgress, isMounted]);
  
  // 渲染白色旋转点
  const renderDots = (config: any) => {
    const { dots, dotsCount, dotsRadius } = config;
  ...
  // 获取当前尺寸的配置
  const currentSizeConfig = sizesConfig[size];
  
// 彩色A Logo渲染 - 缓存组件减少重渲染
const ColorfulALogoRenderer = memo(({ 
  size, 
  currentImageIndex, 
  dotsConfig 
}: { 
    ...
    <div className="relative h-full w-full flex items-center justify-center">
      ...
      >
        ...
        <img 
          ...
          loading="eager" // 确保优先加载
        />
        
        {/* 白点旋转效果 */}
        <div className="absolute inset-0 flex items-center justify-center">
          {renderDots(dotsConfig)}
        </div>
      </motion.div>
    </div>
  );
  ...
  if (displayMode === 'inline') {
    ...
    return (
      <motion.div 
        ...
        aria-live={ariaLive}
      >
        <div className={cn(currentSizeConfig.logo)}>
          <ColorfulALogoRenderer 
            size={size} 
            currentImageIndex={currentImageIndex} 
            dotsConfig={currentSizeConfig} 
          />
        </div>
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div 
            ...
            aria-live={ariaLive}
          >
            <div className={cn(currentSizeConfig.logo, "mb-8")}>
              <ColorfulALogoRenderer 
                size={size} 
                currentImageIndex={currentImageIndex} 
                dotsConfig={currentSizeConfig} 
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            aria-live={ariaLive}
          >
            <div className={cn(currentSizeConfig.logo, "mb-6")}>
              <ColorfulALogoRenderer 
                size={size} 
                currentImageIndex={currentImageIndex} 
                dotsConfig={currentSizeConfig} 
...
// 懒加载版本 - 使用直接解析而不是动态导入
export const LazyUnifiedLoader = lazy(async () => {
  return Promise.resolve({ default: UnifiedLoaderComponent });
});
